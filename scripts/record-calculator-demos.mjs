...
const demos=[
 ...
 {id:'concentration-volume',path:'/tools/dose',title:'Use known concentration to check an entered amount',captions:['0 to 6 seconds: Enter a known concentration of 3 mg/mL.','6 to 14 seconds: Enter 300 mcg as an arithmetic example.','14 to 22 seconds: The result is 0.1 mL, equal to 10 U-100 units.','22 to 30 seconds: The calculation does not choose a treatment, preparation or device.'],run:async(p)=>{await p.getByLabel('Known concentration (mg/mL)').fill('3');await p.waitForTimeout(4000);await p.getByLabel('Amount for one time (mcg)',{exact:true}).fill('300');await expect(p.getByRole('status').filter({hasText:'0.1 mL'})).toBeVisible();await p.waitForTimeout(12000);}},
];
const manifest=[];
