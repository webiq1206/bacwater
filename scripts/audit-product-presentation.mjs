...
for(const [engine,driver] of [['chromium',chromium],['webkit',webkit]]){
 ...
 const p=await c.newPage();p.on('pageerror',e=>errors.push({engine,error:String(e)}));
 try{
  await check(`${engine}: canonical names and complete readable details for every product`,async()=>{
   await p.goto(origin+'/recommendations');
   ...
   for(const [id,name] of Object.entries(names)){
    ...
    await fit(p,d);await p.keyboard.press('Escape');await expect(card.getByRole('button',{name:/Read research details/})).toBeFocused();
   }
  });
  await check(`${engine}: scientific-name searches return partner titles on all product surfaces`,async()=>{
   await p.goto(origin+'/recommendations');await p.getByRole('searchbox',{name:'Find a product',exact:true}).fill('Retatrutide');
   await expect(p.locator('[data-product="glp-3"] h3')).toHaveText('GLP-3 (RT)');
   await p.goto(origin+'/search');await p.getByRole('searchbox',{name:'What are you looking for?'}).fill('tirzepetide');
   await expect(p.locator('[data-search-result="product:glp-2"] strong')).toHaveText('GLP-2 (TR)');
   await p.goto(origin+'/peptide-calculator');await p.getByRole('combobox',{name:'Product',exact:true}).click();
   ...
   for(const route of ['/calculate/retatrutide','/calculate/product/glp-3','/peptides/retatrutide']){await p.goto(origin+route);await expect(p.locator('h1')).toContainText('GLP-3 (RT)');}
  });
  await check(`${engine}: desktop, phone, long names and short screens keep content and actions reachable`,async()=>{
   for(const [width,height,id] of [[1440,1000,'glp-3'],[390,844,'glp-3'],[320,568,'cjc-ipa-no-dac'],[430,932,'bpc-tb-spray'],[844,390,'glp-3']]){
    await p.setViewportSize({width,height});await p.goto(origin+'/recommendations');await p.getByRole('searchbox',{name:'Find a product',exact:true}).fill(id);
    ...
    await fit(p,d);await a11y(p);await p.screenshot({path:`${out}/${engine}-details-${id}-${width}.png`,fullPage:false});
  ...
  await check(`${engine}: directory, enlarged text and no obsolete review section`,async()=>{
   await p.setViewportSize({width:1440,height:1000});await p.goto(origin+'/recommendations');await a11y(p);await p.screenshot({path:`${out}/${engine}-directory-desktop.png`,fullPage:false});
 ...
 }catch(e){process.exitCode=1;console.error(e);await p.screenshot({path:`${out}/${engine}-failure.png`,fullPage:true}).catch(()=>{});await fs.writeFile(`${out}/${engine}-failure.html`,await p.content().catch(()=>''));}finally{await b.close();}
}
if(errors.length||external.some(u=>new URL(u).hostname.endsWith('aminoclub.com')))process.exitCode=1;
await fs.writeFile(`${out}/results.json`,JSON.stringify({date:new Date().toISOString(),results,errors,external,limitations:['Test database and browser emulation only, not the live deployment or physical devices.','External requests are blocked; no referral clicks, purchases, stock checks or conversion-credit claims.','Readability checks are editorial and structural, not a certified school reading-grade assessment.']},null,2));
