...
try {
  ...
  await step('Enter label inputs and save a guest plan',async()=>{
    ...
    await page.getByRole('radio',{name:/^Each time/}).check();await page.getByLabel('Amount from your instructions',{exact:true}).fill('0.4');
  ...
  await step('Deleted account session never reveals another account or admin records', async()=>{
  ...
  await prisma.user.deleteMany({where:{OR:[{email},{id:{in:userIds}}]}});
