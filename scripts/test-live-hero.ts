...
test("new hero does not send raw entries to analytics or external services",()=>{const src=fs.readFileSync("src/components/brand/hero-calculator.tsx","utf8");assert.equal(/\bfetch\(|\bgtag\(|dataLayer|sendBeacon/.test(src),false);assert.ok(src.includes("useCalculationSession"));const session=fs.readFileSync("src/lib/session/calculation-session.ts","utf8");assert.ok(session.includes("sessionStorage"));assert.equal(/\bfetch\(|\bgtag\(|dataLayer|sendBeacon/.test(session),false);assert.equal(HERO_DRAFT_KEY,"bacwater.heroDraft.v1");});
