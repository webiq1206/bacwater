...
test("new hero does not send raw entries to analytics or external services",()=>{const src=fs.readFileSync("src/components/brand/hero-calculator.tsx","utf8");assert.equal(/\bfetch\(|\bgtag\(|dataLayer|sendBeacon/.test(src),false);assert.ok(src.includes("useMassDraft"));assert.ok(fs.readFileSync("src/lib/session/use-session-draft.ts","utf8").includes("window.sessionStorage"));assert.equal(HERO_DRAFT_KEY,"bacwater.heroDraft.v1");});
