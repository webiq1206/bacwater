...
try {
  ...
  await step('Reject negative values without displaying a valid result',async()=>{await page.getByLabel('U-100 syringe units',{exact:true}).fill('-1');await expect(page.getByRole('alert')).toContainText('non-negative');await expect(page.getByLabel('Milliliters (mL)',{exact:true})).toHaveValue('');});
