...
for(const [engine,driver] of [['chromium',chromium],['webkit',webkit]]){
 ...
 try{
  ...
  await check(`${engine}: live concentration, optional measurement, units, invalid input and copy`,async()=>journey(1440,1000,async p=>{
   ...
   await calc.getByText('Amount each time & schedule',{exact:true}).click();await calc.getByRole('button',{name:'mcg',exact:true}).click();await calc.getByLabel('Amount each time',{exact:true}).fill('300');await expect(result).toContainText('0.1 mL');
   ...
   await calc.getByRole('button',{name:'mg',exact:true}).click();await expect(calc.getByLabel('Amount each time',{exact:true})).toHaveValue('0.3');await expect(result).toContainText('0.15 mL');
   await calc.getByRole('button',{name:'Copy result',exact:true}).click();assert.match(await p.evaluate(()=>window.__copied),/0\.15 mL/);
   await calc.getByLabel('Final liquid volume',{exact:true}).fill('0');await expect(calc.getByRole('alert').first()).toContainText('Check your entries');await expect(result).not.toContainText('0.15 mL');await expect(calc.getByRole('button',{name:'Copy result',exact:true})).toBeDisabled();
   await calc.getByLabel('Final liquid volume',{exact:true}).fill('4');await calc.getByLabel('Amount each time',{exact:true}).fill('-1');await expect(calc.getByRole('alert').first()).toContainText('greater than zero');await expect(result).not.toContainText('U-100 units');
  ...
  await check(`${engine}: mobile editing enters a full-screen calculator with persistent results`,async()=>journey(390,844,async p=>{
   ...
   await focus.getByText('Amount each time & schedule',{exact:true}).click();await focus.getByRole('button',{name:'mcg',exact:true}).click();await focus.getByLabel('Amount each time',{exact:true}).fill('300');await expect(focus.locator('[data-hero-result]')).toContainText('0.1 mL');
   ...
   await p.reload({waitUntil:'networkidle'});await expect(p.locator('[data-hero-calculator]').getByLabel('Amount in vial',{exact:true})).toHaveValue('12');await p.getByRole('button',{name:'Open hero calculator full screen'}).click();await expect(focus.getByLabel('Amount each time',{exact:true})).toHaveValue('300');
