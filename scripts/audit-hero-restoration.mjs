...
for(const [engine,driver] of [['chromium',chromium],['webkit',webkit]]){
 ...
 try{
  ...
  await check(`${engine}: live concentration, optional measurement, units, invalid input and copy`,async()=>journey(1440,1000,async p=>{
   ...
   await calc.getByText('Amount & timing: find mL for each use',{exact:true}).click();await calc.getByRole('radio',{name:/^Each time/}).check();await calc.getByRole('combobox',{name:'Amount unit',exact:true}).selectOption('mcg');await calc.getByLabel('Amount from your instructions',{exact:true}).fill('300');await expect(result).toContainText('0.1 mL');
   ...
   await calc.getByLabel('Amount unit',{exact:true}).selectOption('mg');await expect(calc.getByLabel('Amount from your instructions',{exact:true})).toHaveValue('0.3');await expect(result).toContainText('0.15 mL');
   ...
   await calc.getByLabel('Final liquid volume',{exact:true}).fill('4');await calc.getByLabel('Amount from your instructions',{exact:true}).fill('-1');await expect(calc.locator('[data-amount-timing]')).toContainText('positive number');await expect(result).not.toContainText('U-100 units');
  ...
  await check(`${engine}: mobile editing enters a full-screen calculator with persistent results`,async()=>journey(390,844,async p=>{
   ...
   await focus.getByText('Amount & timing: find mL for each use',{exact:true}).click();await focus.getByRole('radio',{name:/^Each time/}).check();await focus.getByRole('combobox',{name:'Amount unit',exact:true}).selectOption('mcg');await focus.getByLabel('Amount from your instructions',{exact:true}).fill('300');await expect(focus.locator('[data-hero-result]')).toContainText('0.1 mL');
   ...
   await p.reload({waitUntil:'networkidle'});await expect(p.locator('[data-hero-calculator]').getByLabel('Amount in vial',{exact:true})).toHaveValue('12');await p.getByRole('button',{name:'Open hero calculator full screen'}).click();await expect(focus.getByLabel('Amount from your instructions',{exact:true})).toHaveValue('300');
