...
async function check(name, run) {
  ...
  catch (error) { results.push({ name, status: 'failed', error: String(error) }); process.exitCode = 1; }
}
for (const [engine, driver] of [['chromium', chromium], ['webkit', webkit]]) {
  ...
  // Independent snapshots must not abort the previous document's idle prefetches.
  // Monitor errors throughout each real journey, and record deliberate shutdown separately.
  async function withPage(width, height, run) {
    const page = await context.newPage();
    let closing = false;
    page.on('pageerror', error => (closing ? teardownErrors : errors).push({ engine, url: page.url(), error: String(error) }));
    try {
    ...
    if (engine === 'chromium') {
      await check('Default and enlarged typography remain readable without horizontal page overflow', async () => {
        for (const width of [320,390,1440]) {
          await withPage(width, width < 768 ? 844 : 900, async page => {
...
await fs.writeFile(`${out}/results.json`, JSON.stringify({ results, errors, teardownErrors, limitations: ['Tests use isolated fixture data and browser viewport emulation, not physical phones.', 'Enlarged text may extend the hero vertically; content is not clipped to force it into one screen.', 'Deliberate page-close cancellation is recorded separately from active-page runtime errors.', 'Supplier catalog expansion and supplier-name cleanup are separate from this hero-only release.'] }, null, 2));
