...
const productUrl=id=>`https://www.aminoclub.com/us/products/${id}?utm_source=affiliate_marketing&code=WEBIQ`;
try{
 ...
 await check('Every supplier shelf includes all 50 listings and owner-approved affiliate attribution',async()=>{for(const route of ['/']){await p.goto(origin+route);const shelf=p.locator('[data-supplier-shelf]');await expect(shelf).toHaveCount(1);assert.equal(await shelf.locator('[data-product]').count(),50);assert.deepEqual(await shelf.locator('[data-product]').evaluateAll(els=>els.slice(0,7).map(e=>e.dataset.product)),expected);for(const slug of expected){const a=shelf.locator(`[data-product="${slug}"] a[target="_blank"]`);await expect(a).toHaveAttribute('href',productUrl(slug));await expect(a).toHaveAttribute('referrerpolicy','no-referrer');await expect(a).toHaveAttribute('rel',/sponsored/);}await expect(shelf.locator('[data-bac-water-link] a')).toHaveAttribute('href',productUrl('amino-h2o'));}});
 await check('Carousel arrows and keyboard move cards; water link stays outside track',async()=>{await p.goto(origin);const track=p.getByRole('region',{name:'Research products'}).getByLabel('Scrollable product cards');await p.getByRole('button',{name:'Next supplier products'}).click();await expect.poll(()=>track.evaluate(el=>el.scrollLeft)).toBeGreaterThan(100);await track.focus();await p.keyboard.press('End');await expect(p.getByRole('button',{name:'Next supplier products'})).toBeDisabled();await p.keyboard.press('Home');await expect(p.getByRole('button',{name:'Previous supplier products'})).toBeDisabled();assert.equal(await p.locator('[data-supplier-shelf] [data-bac-water-link]').evaluate(el=>Boolean(el.closest('[aria-label="Scrollable product cards"]'))),false);});
 await check('Reference pages retain a compact directory path without repeated product cards',async()=>{for(const route of ['/tools','/peptides/bpc-157','/learn','/tools/vial-labels']){await p.goto(origin+route);await expect(p.locator('[data-supplier-shelf]')).toHaveCount(0);await expect(p.locator('[data-research-directory-link] a')).toHaveAttribute('href','/recommendations');await expect(p.locator('[data-research-directory-link]')).toContainText('commission');}});
 ...
 await check('Directory remains in sitemap and no vendor scripts load',async()=>{const r=await c.request.get(origin+'/sitemap-pages.xml');assert.ok((await r.text()).includes('/recommendations</loc>'));assert.equal(external.some(u=>new URL(u).hostname.endsWith('aminoclub.com')),false);assert.deepEqual(errors,[]);});
