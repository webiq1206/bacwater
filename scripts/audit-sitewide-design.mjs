...
try{
 await check('Homepage calculator stays functional and precedes supplier cards',async()=>{await p.goto(origin);await p.getByLabel('Amount on the label',{exact:true}).fill('12');await p.getByLabel('Total liquid after mixing',{exact:true}).fill('6');await expect(p.locator('#quick-calculator')).toContainText('12 mg ÷ 6 mL = 2 mg/mL');const pos=await p.evaluate(()=>document.querySelector('#quick-calculator').compareDocumentPosition(document.querySelector('[data-supplier-shelf]')));assert.ok(pos&4);});
 await check('Every supplier shelf includes all seven exact product destinations',async()=>{for(const route of ['/','/tools','/tools/bac-water','/peptides/bpc-157','/learn','/recommendations']){await p.goto(origin+route);const shelf=p.locator('[data-supplier-shelf]');await expect(shelf).toHaveCount(1);assert.deepEqual(await shelf.locator('[data-product]').evaluateAll(els=>els.map(e=>e.dataset.product)),expected);for(const slug of expected){const a=shelf.locator(`[data-product="${slug}"] a`);await expect(a).toHaveAttribute('href',`https://www.aminoclub.com/us/products/${slug}`);await expect(a).toHaveAttribute('referrerpolicy','no-referrer');await expect(a).toHaveAttribute('rel',/sponsored/);}await expect(shelf.locator('[data-bac-water-link] a')).toHaveAttribute('href','https://www.aminoclub.com/us/products/amino-h2o');}});
