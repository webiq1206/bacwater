...
try{
 ...
 await check('Every supplier shelf includes all 50 product listings and the featured destinations',async()=>{for(const route of ['/','/tools','/recommendations']){await p.goto(origin+route);const shelf=p.locator('[data-supplier-shelf]');await expect(shelf).toHaveCount(1);assert.equal(await shelf.locator('[data-product]').count(),50);assert.deepEqual(await shelf.locator('[data-product]').evaluateAll(els=>els.slice(0,7).map(e=>e.dataset.product)),expected);for(const slug of expected){const a=shelf.locator(`[data-product="${slug}"] a[target="_blank"]`);await expect(a).toHaveAttribute('href',`https://www.aminoclub.com/us/products/${slug}?utm_source=affiliate_marketing&code=WEBIQ`);await expect(a).toHaveAttribute('referrerpolicy','no-referrer');await expect(a).toHaveAttribute('rel',/sponsored/);}await expect(shelf.locator('[data-bac-water-link] a')).toHaveAttribute('href','https://www.aminoclub.com/us/products/amino-h2o?utm_source=affiliate_marketing&code=WEBIQ');}});
 await check('Clinical and editorial pages do not promote supplier products',async()=>{for(const route of ['/peptides/bpc-157','/learn','/editorial-policy','/methodology']){await p.goto(origin+route);await expect(p.locator('[data-supplier-shelf]')).toHaveCount(0);await expect(p.locator('a[href*="aminoclub.com/us/products/"]')).toHaveCount(0);}});
