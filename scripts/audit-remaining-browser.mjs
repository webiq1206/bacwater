...
try{
 ...
 await step('Known concentration converts both directions without implicit dosing',async()=>{
  await page.goto(`${origin}/tools/dose`);await page.getByLabel('Known concentration (mg/mL)').fill('3');await page.getByLabel('Amount for one time (mcg)').fill('300');await expect(page.getByRole('status').filter({hasText:'U-100'})).toContainText('0.1 mL');
 ...
 await step('Reverse volume and inventory use explicitly entered values',async()=>{
  await page.goto(`${origin}/tools/reverse-bac`);await page.getByLabel('Total mass (mg)').fill('10');await page.getByLabel('Amount for one time (mcg)').fill('500');await page.getByLabel('Hypothetical U-100 reading (units)').fill('10');await expect(page.getByRole('status').filter({hasText:'Hypothetical final volume'})).toContainText('2 mL');
  await page.goto(`${origin}/tools/supplies`);await page.getByLabel('Mass per vial (mg)').fill('10');await page.getByLabel('Amount for one time (mcg)').fill('500');await page.getByLabel('Number of measurements').fill('41');await expect(page.getByRole('status').filter({hasText:'complete measurements'})).toContainText('3 vials');await page.getByLabel('Number of measurements').fill('41.5');await expect(page.getByRole('status').filter({hasText:'whole count'})).toBeVisible();
 });
 await step('Compound calculator retains compatible vial entries and distinguishes hCG IU from mass',async()=>{
  await page.goto(`${origin}/peptides/bpc-157`);await page.getByRole('link',{name:'Open BPC-157 calculator',exact:true}).click();await expect(page.getByLabel('Total in container (mg)',{exact:true})).toHaveValue('10');await page.getByLabel('Total in container (mg)',{exact:true}).fill('10');await page.getByLabel('Final volume (mL)',{exact:true}).fill('2');await page.getByLabel('Amount for one time',{exact:true}).fill('400');await expect(page.getByRole('status').filter({hasText:'5 mg/mL'})).toContainText('8 U-100');
  await page.getByLabel('Total in container (mg)',{exact:true}).fill('0x10');await expect(page.locator('.bac-result-card')).not.toContainText('5 mg/mL');await expect(page.locator('.bac-result-card')).toContainText('Enter the total amount');
