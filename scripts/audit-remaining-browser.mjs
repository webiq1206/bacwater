...
try{
 ...
 await step('Reverse volume and inventory use explicitly entered values',async()=>{
  await page.goto(`${origin}/tools/reverse-bac`);await page.getByLabel('Total mass (mg)').fill('10');await page.getByLabel('Amount for one time (mcg)',{exact:true}).fill('500');await page.getByLabel('Hypothetical U-100 reading (units)').fill('10');await expect(page.getByRole('status').filter({hasText:'Hypothetical final volume'})).toContainText('2 mL');
