...
try{
 ...
 await step('Compound calculator is blank and distinguishes hCG IU from mass',async()=>{
  await page.goto(`${origin}/peptides/bpc-157`);await page.getByRole('link',{name:'Open BPC-157 calculator',exact:true}).click();await expect(page.getByLabel('Total in container (mg)',{exact:true})).toHaveValue('');await page.getByLabel('Total in container (mg)',{exact:true}).fill('10');await page.getByLabel('Final volume (mL)',{exact:true}).fill('2');await page.getByLabel('Entered amount (mcg)',{exact:true}).fill('400');await expect(page.getByRole('status').filter({hasText:'5 mg/mL'})).toContainText('8 U-100');
