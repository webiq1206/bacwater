...
import {CORE_BACWATER_REFERENCES,sourceReference} from "@/lib/content/references";
...
export default async function PeptidePage({params}:{params:Promise<{slug:string}>}){
 ...
 const refs=[...CORE_BACWATER_REFERENCES,...(content?.sources??[]).map(sourceReference)];
 ...
 <details className="mt-4 rounded-xl border p-4"><summary className="cursor-pointer min-h-11 font-medium">What this reference cannot verify</summary><VialIdentityWarning compound={short}/>{!custom&&<WhatNobodyKnows compound={short} evidence={evidenceOf(p)}/>}</details><p className="mt-4 text-sm text-muted-foreground">Product names follow the partner’s catalog. The scientific references describe compounds and study materials, not proof for a supplier’s product.</p>
 ...
 {related.length>0&&<section className="mt-10"><h2 className="text-xl font-serif">Related compound references</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{related.map(r=><Link key={r.slug} href={`/peptides/${r.slug}`} className="min-w-0 rounded-lg border p-4 font-medium hover:bg-muted">{productDisplayName(r.slug, shortName(r.name))}</Link>)}</div><Link href={`/peptides/compare?a=${p.slug}&b=${related[0].slug}`} className="mt-4 inline-block text-sm underline">Compare the label context of {short} and {productDisplayName(related[0].slug, shortName(related[0].name))}</Link></section>}
 <References references={refs}/>
 <section className="section-dark mt-10 rounded-2xl p-5 sm:p-7"><h2 className="text-xl font-serif">Keep arithmetic separate from product instructions.</h2><p className="mt-3 text-sm leading-relaxed">{iu ? 'Use the dedicated hCG calculator for activity in IU. Product IU and U-100 volume markings are different units.' : 'Save your entered mass calculation and print a record. A saved calculation is not a preparation prescription.'}</p><div className="mt-4 flex flex-wrap gap-3"><Button asChild variant="brand"><Link href={iu?'/calculate/hcg':'/plan'}>{iu?'Open hCG IU calculator':'Build a saved calculation'}</Link></Button><Button asChild variant="outline"><Link href="/tools">All calculators</Link></Button></div><p className="mt-4 text-sm"><Link href="/methodology" className="underline">Methodology</Link> · <Link href="/faq" className="underline">Common questions</Link> · <Link href="/learn/what-you-cannot-know" className="underline">What cannot be verified</Link></p></section>
