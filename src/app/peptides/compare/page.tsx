...
export default async function ComparePage({searchParams}:{searchParams:Promise<Params>}){
 ...
 <WebPageJsonLd name={`${productDisplayName(a.slug, shortName(a.name))} vs ${productDisplayName(b.slug, shortName(b.name))}`} description="Compare label context and limits, not treatment choices." url="/peptides/compare"/>
 <Breadcrumbs items={[{label:"Home",href:"/"},{label:"Compounds",href:"/peptides"},{label:"Compare",href:"/peptides/compare"}]}/>
 <p className="eyebrow">Reference comparison</p><h1 className="mt-2 text-3xl sm:text-5xl font-serif">{productDisplayName(a.slug, shortName(a.name))} vs {productDisplayName(b.slug, shortName(b.name))}</h1>
 ...
 <div className="mt-6"><CompareSelector options={options.map(p=>({slug:p.slug,name:productDisplayName(p.slug, shortName(p.name))}))} a={a.slug} b={b.slug}/></div>
 <div className="mt-7 overflow-x-auto rounded-xl border" role="region" aria-label="Compound reference comparison" tabIndex={0}><table className="w-full text-sm text-left"><caption className="sr-only">Label context and limits for the two selected compounds</caption><thead><tr><th scope="col" className="p-3">Reference question</th>{[a,b].map((p,i)=><th key={i} scope="col" className="p-3"><Link className="underline" href={`/peptides/${p.slug}`}>{productDisplayName(p.slug, shortName(p.name))}</Link></th>)}</tr></thead><tbody>{rows.map(row=><tr key={row.label} className="border-t align-top"><th scope="row" className="p-3 font-medium">{row.label}</th>{row.values.map((value,i)=><td key={i} className="p-3 leading-relaxed">{value}</td>)}</tr>)}</tbody></table></div>
 ...
 <div className="mt-6 grid gap-3 sm:grid-cols-2">{[a,b].map((p,i)=><Link key={i} className="rounded-xl border p-4 font-medium hover:bg-muted" href={`/peptides/${p.slug}`}>Open {productDisplayName(p.slug, shortName(p.name))} calculator and references</Link>)}</div>
