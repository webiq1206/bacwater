...
import { useSessionState } from "@/lib/use-calculator-session";
...
export default function MgMcgConverterPage() {
  ...
  const [entry, setEntry] = useSessionState<Entry>(KEY, empty);
  ...
  <SupplyChecklist/></>}>
    <section className="rounded-2xl border border-border bg-card p-5 sm:p-7" aria-label="Mass conversion">
      ...
      <p id="mass-help" className="mt-3 text-sm text-muted-foreground">Use a decimal point, without commas. Your last entry stays in this browser tab when storage is available. No account is needed.</p>
