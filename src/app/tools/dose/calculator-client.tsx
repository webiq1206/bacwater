...
export default function DoseCalculatorPage(){
 ...
 const converted=convertMassText(shared.vialInput,shared.vialUnit),mass=converted.kind==="value"?positiveDecimal(converted.mg):null,volume=positiveDecimal(shared.finalVolume);
 const fingerprint=JSON.stringify([shared.productId,shared.vialInput,shared.vialUnit,shared.finalVolume]);
 const [override,setOverride]=useSessionDraft("known-concentration",{fingerprint:"",text:""});
 const derived=mass!==null&&volume!==null&&Number.isFinite(mass/volume)?String(mass/volume):"";
 const conc=override.fingerprint===fingerprint?override.text:derived;
 const setConc=(text:string)=>setOverride({fingerprint,text});
 const [sampleVolume,setSampleVolume]=useSessionDraft("known-sample-volume","");
 const value=mode!=="mass"?eachAmountText(shared):sampleVolume;
 const setValue=(text:string)=>{if(mode!=="mass")shared.patch({amount:text,amountUnit:"mcg",basis:"each"});else setSampleVolume(text);};
 ...
 return <CalculationPanel slug="dose" title="Amount-to-volume calculator" description="Know the amount in each mL? Put it here. We’ll find a volume or amount from your numbers. We do not choose a dose." fields={[{id:"known-concentration",label:"Amount-to-volume (mg/mL)",text:conc,set:setConc,hint:"A known concentration from your current vial values comes with you. You can enter the label concentration instead; that override applies only to this tool."},{id:"known-amount",label:volumeMode?"Amount for one time (mcg)":"Measured volume (mL)",text:value,set:setValue}]} result={result} onClear={()=>{setConc("");setValue("");}} controls={<div role="group" aria-label="What to calculate" className="mb-5 flex flex-wrap gap-2">{[["volume","Find volume from amount"],["mass","Find amount from volume"]].map(([m,label])=><button type="button" key={m} aria-pressed={(volumeMode?"volume":"mass")===m} className="min-h-11 rounded-xl border px-4 py-2 text-sm aria-pressed:font-bold aria-pressed:underline" onClick={()=>setMode(m)}>{label}</button>)}</div>}>
