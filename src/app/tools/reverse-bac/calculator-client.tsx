...
export default function ReverseBacCalculatorPage(){
 ...
 return <CalculationPanel slug="reverse-bac" title="Reverse BAC water calculation" description="Try the math in reverse. Enter an amount and a U-100 reading. This shows an example volume, not mixing instructions." fields={[{id:"reverse-mass",label:"Total mass (mg)",text:mass,set:setMass},{id:"reverse-amount",label:"Amount for one time (mcg)",text:amount,set:setAmount,hint:"A daily or weekly total from the guided calculator is already split here. Editing this field sets the amount for one time."},{id:"reverse-units",label:"Hypothetical U-100 reading (units)",text:units,set:setUnits}]} result={{ready,text}} onClear={()=>{setMass("");setAmount("");setUnits("");}}>
