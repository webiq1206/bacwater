...
export default function ReverseBacCalculatorPage(){
 const { mass, setMass, amount, setAmount, clear } = useSharedNumbers();
 const [units,setUnits]=usePersistentState("mass-tool:reverse.units","");
 ...
 return <CalculationPanel slug="reverse-bac" title="Reverse BAC water calculation" description="Try the math in reverse. Enter an amount and a U-100 reading. This shows an example volume, not mixing instructions." fields={[{id:"reverse-mass",label:"Total mass (mg)",text:mass,set:setMass},{id:"reverse-amount",label:"Amount each time (mcg)",text:amount,set:setAmount},{id:"reverse-units",label:"Hypothetical U-100 reading (units)",text:units,set:setUnits}]} result={{ready,text}} controls={<SessionNote/>} onClear={()=>{clear();setUnits("");}}>
