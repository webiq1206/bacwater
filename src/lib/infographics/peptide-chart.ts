...
export function hasChart(p: PeptideRef): boolean {
  return p.slug !== "custom" && p.slug !== "hcg" && p.commonVialStrengthsMg.length > 0;
}
...
export function peptideChartSvg(p: PeptideRef): string {
  ...
  const bars = rows
    .map((r, i) => {
      ...
      const label = `${r.vialMg} mg in ${r.bacMl} mL final volume`;
      return `
  ...
  const inner = `
    ${logo(32, 46)}
    <text x="${width - 32}" y="43" text-anchor="end" font-size="11" font-weight="600" letter-spacing="1.5" fill="${PALETTE.accent}">ARITHMETIC EXAMPLES</text>
    ...
    <text x="32" y="124" font-size="13" fill="${PALETTE.muted}">Concentration you get at each illustrative vial amount and final volume.</text>
    ${bars}
    <text x="32" y="${height - 20}" font-size="11" fill="${PALETTE.muted}">Concentration = vial amount &#247; final volume. Longer bar = stronger liquid. Examples are not mixing instructions.</text>
  `;
