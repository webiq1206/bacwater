...
export function ProductPicker({value,onChange,label="Product",referencesOnly=false,excludeValue}:{value:string;onChange:(value:string)=>void;label?:string;referencesOnly?:boolean;excludeValue?:string}){
 ...
 const filtered=options.map((p,i)=>({p,i,score:searchScore(query,p.name,`${p.product?.id||""} ${p.product?.reference||""} ${p.description}`)})).filter(x=>!blocked&&x.score>0).sort((a,b)=>b.score-a.score||a.i-b.i).map(x=>x.p);
 return <><Dialog open={open} onOpenChange={v=>{setOpen(v);if(v)setQuery("");}}><DialogTrigger asChild><button type="button" role="combobox" aria-label={label} aria-haspopup="dialog" aria-expanded={open} aria-controls={`${uid}-picker`} className={styles.trigger} data-product-picker data-selected-product={selected?.product?.id}>
  ...
  </button>)}{!filtered.length&&<p className={styles.noResults}>{blocked?RESEARCH_SEARCH_BOUNDARY:"No match. Try a shorter name, or clear the search and choose Other / Custom."}</p>}</div>
  <p className={styles.footer}>Our illustrations help you find items. They are not photos of product packaging.</p>
 </DialogContent></Dialog>{destination&&<a className={styles.productLink} href={destination.href} target="_blank" rel="sponsored nofollow noopener noreferrer" referrerPolicy="no-referrer" data-selected-product-link>View supplier listing<span aria-hidden="true"> ↗</span></a>}{destination&&<p className={styles.footer} data-affiliate-disclosure>{destination.paid&&AFFILIATE_DISCLOSURE} For laboratory research only. Not for human consumption or animal use.</p>}</>;
}
