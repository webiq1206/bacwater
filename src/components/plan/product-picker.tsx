...
export function ProductPicker({value,onChange,label="Product",referencesOnly=false,excludeValue}:{value:string;onChange:(value:string)=>void;label?:string;referencesOnly?:boolean;excludeValue?:string}){
 ...
 const filtered=options.map((p,i)=>({p,i,score:searchScore(query,p.name,`${p.product?.id||""} ${p.product?.reference||""} ${p.description}`)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score||a.i-b.i).map(x=>x.p);
 ...
 </DialogContent></Dialog>{destination&&<a className={styles.productLink} href={destination.href} target="_blank" rel="sponsored nofollow noopener noreferrer" referrerPolicy="no-referrer" data-selected-product-link>View product details<span aria-hidden="true"> ↗</span></a>}{destination?.paid&&<p className={styles.footer}>Paid link. We may earn a fee.</p>}</>;
}
