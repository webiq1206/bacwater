...
export function PeptideCalc(props:Props){
 if (props.standalone && props.peptideSlug!=="hcg") return <MassCalculator title={`${props.peptideName} calculator`} productId={productForReference(props.peptideSlug || "")?.id || ""} peptideSlug={props.peptideSlug || ""} backHref={`/peptides/${props.peptideSlug || "custom"}`}/>;
 return <ReferenceCalculator {...props}/>;
}
function ReferenceCalculator({peptideName,peptideSlug,standalone=false}:Props){
 const iu=peptideSlug==="hcg";const [mass,setMass]=useSessionValue<string>(`bacwater.compound.${peptideSlug}.mass`,"");const [volume,setVolume]=useSessionValue<string>(`bacwater.compound.${peptideSlug}.volume`,"");const [amount,setAmount]=useSessionValue<string>(`bacwater.compound.${peptideSlug}.amount`,"");
