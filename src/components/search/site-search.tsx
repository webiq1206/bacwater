...
export function SiteSearchResults({onNavigate,standalone=false}:{onNavigate?:()=>void;standalone?:boolean}){
 ...
 const matched=useMemo(()=>searchItems(items,query,kind).filter(item=>!blocked||(item.kind!=="product"&&item.href!=="/products")),[items,query,kind,blocked]);
 ...
 return <div className={styles.searchBody} ref={root} data-site-search-results data-standalone={standalone}>
  ...
  {blocked&&<p className={styles.footnote} data-research-boundary>{RESEARCH_SEARCH_BOUNDARY}</p>}
  <div className={styles.results} aria-label="Search results" tabIndex={0}>
   {visible.length?<ul>{visible.map(item=><li key={item.id}><Link href={item.href} className={styles.result} data-search-result={item.id} onClick={onNavigate} onKeyDown={e=>{if(!["ArrowDown","ArrowUp"].includes(e.key))return;const links=Array.from(root.current?.querySelectorAll<HTMLAnchorElement>("[data-search-result]")||[]),n=links.indexOf(e.currentTarget);e.preventDefault();if(e.key==="ArrowUp"&&n===0)input.current?.focus();else links[Math.max(0,Math.min(links.length-1,n+(e.key==="ArrowDown"?1:-1)))]?.focus();}}><SearchThumbnail item={item}/><span className={styles.resultText}><small>{SEARCH_KIND_LABEL[item.kind]}</small><strong><Highlight text={item.title} query={query}/></strong><span>{item.description}</span><b className={styles.openLabel}>{item.kind==="product"?"Explore product":item.kind==="calculator"?"Open calculator":item.kind==="reference"?"Read reference":"Open page"}</b></span><ArrowRight size={18} aria-hidden="true"/></Link>{!blocked&&item.kind==="product"&&products.find(p=>p.id===item.productId)&&<div className={styles.supplierGroup}><p data-affiliate-disclosure>{products.find(p=>p.id===item.productId)!.paid&&AFFILIATE_DISCLOSURE} For laboratory research only. Not for human consumption or animal use.</p><a className={styles.supplierAction} href={products.find(p=>p.id===item.productId)!.href} target="_blank" rel="sponsored nofollow noopener noreferrer" referrerPolicy="no-referrer" aria-label={`View ${item.title} product, opens a new tab`}>View supplier listing <span aria-hidden="true">↗</span></a></div>}</li>)}</ul>:<div className={styles.empty}><Search size={30} aria-hidden="true"/><h3>No matches yet</h3><p>Try a shorter name, different spelling, or the All filter.</p><Link href="/contact" onClick={onNavigate}>Ask us for help</Link></div>}
