...
export function SiteSearchResults({onNavigate,standalone=false}:{onNavigate?:()=>void;standalone?:boolean}){
 ...
 return <div className={styles.searchBody} ref={root} data-site-search-results data-standalone={standalone}>
  ...
  <div className={styles.results} aria-label="Search results" tabIndex={0}>
   {visible.length?<ul>{visible.map(item=><li key={item.id}><Link href={item.href} className={styles.result} data-search-result={item.id} onClick={onNavigate} onKeyDown={e=>{if(!["ArrowDown","ArrowUp"].includes(e.key))return;const links=Array.from(root.current?.querySelectorAll<HTMLAnchorElement>("[data-search-result]")||[]),n=links.indexOf(e.currentTarget);e.preventDefault();if(e.key==="ArrowUp"&&n===0)input.current?.focus();else links[Math.max(0,Math.min(links.length-1,n+(e.key==="ArrowDown"?1:-1)))]?.focus();}}><SearchThumbnail item={item}/><span className={styles.resultText}><small>{SEARCH_KIND_LABEL[item.kind]}</small><strong><Highlight text={item.title} query={query}/></strong><span>{item.description}</span><b className={styles.openLabel}>{item.kind==="product"||item.kind==="calculator"?"Open calculator":item.kind==="reference"?"Read reference":"Open page"}</b></span><ArrowRight size={18} aria-hidden="true"/></Link>{item.kind==="product"&&products.find(p=>p.id===item.productId)&&<><p style={{fontSize:13,lineHeight:1.6,margin:"8px 16px"}}>{products.find(p=>p.id===item.productId)!.paid?AFFILIATE_DISCLOSURE:"Supplier link. No paid referral is active."} {RESEARCH_ONLY_NOTICE}</p><a className={styles.supplierAction} href={products.find(p=>p.id===item.productId)!.href} target="_blank" rel="sponsored nofollow noopener noreferrer" referrerPolicy="no-referrer" aria-label={`View ${item.title} product, opens a new tab`}>View product <span aria-hidden="true">↗</span></a></>}</li>)}</ul>:<div className={styles.empty}><Search size={30} aria-hidden="true"/><h3>No matches yet</h3><p>Try a shorter name, different spelling, or the All filter.</p><Link href="/contact" onClick={onNavigate}>Ask us for help</Link></div>}
