...
export function ProductSearchButton({ compact = false }: { compact?: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={next => { if (!next) selectProduct(null); setOpen(next); }}>
    ...
    <DialogContent ref={dialog} className={`${searchStyles.dialog} ${styles.productDialog}`} data-product-search-dialog
      onOpenAutoFocus={event => { event.preventDefault(); input.current?.focus(); }}
      onEscapeKeyDown={event => {
        // A rapid Escape can arrive before the inner layer finishes registering.
        // Close exactly the active details, never both the details and the search.
        if (activeProduct.current !== null) { event.preventDefault(); selectProduct(null); }
      }}
      ...
      <ProductSearchField query={query} onChange={setQuery} match={match} inputRef={input} showInitial previewCount={6}
        activeProductId={activeProductId} onActiveProductChange={selectProduct}/>
      <div className={styles.dialogFooter}><Link href="/recommendations" onClick={() => { selectProduct(null); setOpen(false); }}>Browse the full directory <ArrowRight size={16} aria-hidden="true" /></Link><p>Search stays in your browser. Images are original directory illustrations, not product packaging.</p></div>
    </DialogContent>
  </Dialog>;
}
