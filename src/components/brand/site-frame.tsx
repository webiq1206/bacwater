...
export function SiteFrame({children,shelf}:{children:ReactNode;shelf:ReactNode}) {
 ...
 const showShelf=supplierPromotionAllowed(path)&&!path.startsWith("/products")&&!isCalculatorWorkspace(path)&&path!=="/"&&path!=="/recommendations"&&!path.startsWith("/admin")&&!path.startsWith("/plans/labels")&&!(path!=="/plan/new"&&/^\/plan\/[^/]+(?:\/|$)/.test(path))&&path!=="/signin"&&path!=="/signup";
