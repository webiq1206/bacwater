...
export function HeroCalculator() {
    ...
    return <>
      ...
      <div className={styles.panel} id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-tab-${mode}`}>
        ...
        {mode === "concentration" && <details className={styles.optional} open={measurementOpen}><summary onClick={e => { e.preventDefault(); setMeasurementOpen(v => !v); if (scope === "inline" && smallScreen()) focusScreen(); }}>Also find mL and U-100 units</summary><div className={styles.target}>
          ...
          <label className={styles.unitSelect}>Amount unit<select value={values.targetUnit} onChange={e => { setValues(v => changeTargetUnit(v, e.target.value as "mg" | "mcg")); setExample(false); setNotice(""); }}><option value="mcg">mcg</option><option value="mg">mg</option></select></label>
        </div></details>}
        <div className={styles.result} role="status" aria-live="polite" aria-atomic="true">
          <div className={styles.resultTop}><span>{result?.label || (mode === "concentration" ? "How much is in each mL" : "Your conversion")}</span>{result && <Check size={18} aria-hidden="true" />}</div>
  ...
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    ...
    <Dialog.Portal><Dialog.Overlay className={styles.overlay} /><Dialog.Content ref={dialogRef} className={styles.focus} data-hero-focus
      ...
      <div className={styles.focusBar}><div><Dialog.Title>BAC water calculator</Dialog.Title><Dialog.Description>Just your numbers. Your entries stay when you close this screen.</Dialog.Description></div><Dialog.Close aria-label="Return to homepage"><X size={22} aria-hidden="true" /></Dialog.Close></div>
