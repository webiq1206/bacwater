...
export function HeroCalculator() {
  ...
  const unitValue = unitEntry.direction === "units" ? unitEntry.text : unitEntry.text && Number.isFinite(Number(unitEntry.text)) ? String(Number(unitEntry.text) * 100) : unitEntry.text;
  ...
  function content(scope: "inline" | "focus") {
    ...
    return <>
      ...
      <div className={styles.panel} id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-tab-${mode}`}>
        ...
        <p id={`${id}-error`} className={styles.error} role={invalid || (mode === "concentration" && (measurement.kind === "error" || (!!draft.amount && !schedule.ready))) ? "alert" : undefined}>{invalid ? "Check your entries. Use a decimal point, no commas or unit words." : mode === "concentration" && measurement.kind === "error" ? measurement.message : ""}</p>
