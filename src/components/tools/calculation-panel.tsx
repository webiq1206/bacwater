...
export function CalculationPanel({slug,title,description,fields,result,onClear,children,controls,inputExtra}:{slug:string;title:string;description:string;fields:{id:string;label:string;text:string;set:(value:string)=>void;hint?:string}[];result:{ready:boolean;text:string};onClear:()=>void;children:ReactNode;controls?:ReactNode;inputExtra?:ReactNode}){
...
<section aria-label={title} className="bac-calc-card p-5 sm:p-7">{controls}<div className="grid gap-5 sm:grid-cols-2">{fields.map(f=><div key={f.id}><label className="block font-medium" htmlFor={f.id}>{f.label}</label><Input id={f.id} type="text" inputMode="decimal" value={f.text} onChange={e=>f.set(e.target.value)} maxLength={64} aria-describedby={f.hint?`${f.id}-help`:undefined} className="mt-2 min-h-12" />{f.hint&&<p id={`${f.id}-help`} className="mt-2 text-sm text-muted-foreground">{f.hint}</p>}</div>)}</div>{inputExtra}<div role="status" aria-live="polite" aria-atomic="true" className="bac-result-card mt-5 break-words [overflow-wrap:anywhere]">{result.text}</div><div className="mt-5 flex flex-wrap gap-3"><Button type="button" variant="outline" onClick={onClear}>Clear inputs</Button>{result.ready&&<CopyButton value={result.text+" Arithmetic only. Verify product and device instructions."} label="Copy result" />}</div><p className="mt-3 text-xs text-muted-foreground">Your entries stay in this browser tab. No account is required.</p></section>
 </CalculatorWorkspace>;
}
