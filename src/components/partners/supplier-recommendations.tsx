...
export function ResearchSupplierSection({products=getSupplierCatalog()}:{products?:readonly DisplaySupplierProduct[]}) {
  return <section className={styles.section} data-supplier-shelf aria-label="Research supplier products">
    ...
    <p className={styles.lead}>Looking for lab supplies? Start with the label. Open research details here, or view the supplier's product page.</p>
    ...
    <p className={styles.policy}>{products.some(p=>p.paid)?"We are an independent affiliate and may earn a commission from purchases through these links. ":""}{products.length} products. Check current sizes and stock on the supplier's website.</p>
...
/** Keep reference pages focused; the homepage and directory retain all products. */
  ...
  return <aside data-research-directory-link className="mx-auto mb-12 w-full max-w-5xl px-4 sm:px-6" aria-label="Research product directory">
    <div className="rounded-2xl border border-border bg-surface p-6 sm:flex sm:items-center sm:justify-between sm:gap-8">
      <div className="max-w-2xl">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">Compare product details and labels in the complete directory. Research use only. We have not independently tested the products and may earn a commission from linked purchases.</p>
      </div>
