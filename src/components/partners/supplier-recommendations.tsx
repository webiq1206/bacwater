...
export function ResearchSupplierSection({products=getSupplierCatalog()}:{products?:readonly DisplaySupplierProduct[]}) {
  return <section className={styles.section} data-supplier-shelf aria-label="Research supplier listings">
    ...
    <p className={styles.policy}>For lab research only, not for people or animals. This list is not advice about what to take or mix. We have not tested these products. Use the search to find a product. Your entries do not rank this list.</p>
    <p className={styles.policy}>{products.length} product listings. Check sizes and stock on each product page.</p>
