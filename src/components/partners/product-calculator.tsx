...
import { useCalculationSession, chooseCalculationProduct, setMassUnit } from "@/lib/session/calculation-session";
...
export function ProductCalculator({product}:{product:SupplierProduct}){
 ...
 const values=product.kind==="single"?{...readProductValues(raw),total:converted.kind==="value"?converted.mg:shared.vialInput,volume:shared.finalVolume,amount:eachAmountText(shared)}:readProductValues(raw);
 ...
 function updateField(key:keyof Omit<ProductValues,"ingredients">,value:string){if(product.kind!=="single"){set({...values,[key]:value});return;}if(key==="total")shared.patch({vialInput:value,vialUnit:"mg"});if(key==="volume")shared.patch({finalVolume:value});}
 ...
 return <CalculatorWorkspace title={`${product.name} calculator`} description={description} backHref="/recommendations" help={<p>All products are listed for lab research only, not for people or animals. This calculation does not verify a product or tell you what to use. The supplier link opens the exact product page. Images are our own artwork, not packaging.</p>}>
  {product.kind==="single"&&<SessionValuesNotice/>}<section className="bac-calc-card p-5 sm:p-7" aria-label="Product calculation inputs" style={{maxWidth:760,marginInline:"auto"}}>
   ...
   {product.kind==="single"&&<div className="mt-6 border-t pt-5"><h2 className="mb-3 text-xl font-semibold">How much, and how often?</h2><AmountScheduleFields/></div>}
   <UnitHelp/>
  ...
  <WorkspaceActions><Button type="button" variant="outline" onClick={()=>{if(product.kind==="single")shared.clear();else set(emptyProductValues());}}>Clear inputs</Button>{result.ready&&<CopyButton value={result.text+". Arithmetic only; check product instructions."} label="Copy result"/>}</WorkspaceActions>
 </CalculatorWorkspace>;
}
