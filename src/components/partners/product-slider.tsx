...
import { productCalculatorPath, productDetailPath } from "@/lib/partners/supplier-catalog";
...
export function ProductSlider({products}:{products:readonly DisplaySupplierProduct[]}) {
  ...
  const filtered=products.filter(p=>!blocked&&(kind==="all"||p.kind===kind)&&`${p.name} ${p.id} ${p.mark} ${p.reference}`.toLowerCase().includes(query.trim().toLowerCase()));
  ...
  return <div className={styles.slider} role="region" aria-roledescription="carousel" aria-label="Research products">
    <div className={styles.filters}><label>Find a product<input type="search" maxLength={240} autoComplete="off" data-clarity-mask="true" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search all products" /></label><label>Product type<select aria-label="Product type" value={kind} onChange={e=>setKind(e.target.value)}><option value="all">All products</option><option value="single">Single compounds</option><option value="blend">Blends</option><option value="spray">Sprays and solutions</option><option value="water">Lab water</option></select></label></div>
    ...
    <div id={`${id}-track`} ref={track} className={styles.track} role="group" tabIndex={0} aria-label="Scrollable product cards" aria-describedby={`${id}-help`} onKeyDown={key}>
      {filtered.map((product,index)=><article className={styles.card} key={product.id} data-product={product.id} role="group" aria-roledescription="slide" aria-label={`${index+1} of ${filtered.length}: ${product.name}`}>
        ...
        <div className={styles.cardBody}><p className={styles.label}>{product.label}</p><h3><Link href={productDetailPath(product.id)}>{product.name}</Link></h3><p className={styles.summary}>{product.summary}</p><p className={styles.notice}>Laboratory research only.<br/>Not for human consumption or animal use.</p><p className={styles.disclosure} data-affiliate-disclosure>{product.paid?AFFILIATE_DISCLOSURE:"Supplier link. No referral fee is active."}</p><a className={styles.action} href={product.href} target="_blank" rel="sponsored nofollow noopener noreferrer" referrerPolicy="no-referrer" aria-label={`View ${product.name} from the supplier, opens a new tab`}>View supplier listing <ArrowUpRight size={17} aria-hidden="true"/></a><Link className={styles.calculate} href={productDetailPath(product.id)}>Explore product details</Link><Link className={styles.calculate} href={productCalculatorPath(product.id)}>Use in calculator</Link></div>
      </article>)}
    </div><p className={styles.range} role="status" aria-live="polite">{filtered.length?`Showing ${range.first} to ${range.last} of ${filtered.length}`:blocked?RESEARCH_SEARCH_BOUNDARY:"No products match. Try another name or product type."}</p>
  </div>;
}
