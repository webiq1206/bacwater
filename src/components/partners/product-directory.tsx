...
export function ProductDirectory({ products }: { products: readonly DisplaySupplierProduct[] }) {
  ...
  return <div className={styles.directory} data-product-directory>
    <form role="search" className={styles.searchPanel} onSubmit={event => event.preventDefault()}>
      ...
      <div className={styles.searchField}><Search size={21} aria-hidden="true"/><input id="research-catalog-search" ref={input} type="search" value={query} maxLength={CATALOG_SEARCH_LIMIT} onChange={event => changeQuery(event.target.value)} placeholder="Search research materials" aria-describedby="research-search-help" autoComplete="off" data-clarity-mask="true" spellCheck={false}/>{query && <button type="button" aria-label="Clear product search" onClick={() => { changeQuery(""); input.current?.focus(); }}><X size={20} aria-hidden="true"/></button>}</div>
