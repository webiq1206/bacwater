...
export function AmountScheduleFields({ value, onChange }: { value: AmountSchedule; onChange: (value: AmountSchedule) => void }) {
  ...
  return <div className={styles.root} data-amount-schedule>
    <fieldset className={styles.basis}><legend>What does the amount in your instructions mean?</legend>
    ...
    <div className={styles.amount}>
      ...
      <div className={styles.inputRow}><input id={`${id}-amount`} data-schedule-amount type="text" inputMode="decimal" autoComplete="off" maxLength={64} value={value.amount} onChange={e => update({ amount: e.target.value })} aria-describedby={`${id}-hint ${id}-error`} aria-invalid={!!value.amount && !result.ready}/>
    ...
    <div className={styles.frequency}>
      ...
      {customShown && <div className={styles.custom}><label htmlFor={`${id}-custom`}>Times per week</label><input id={`${id}-custom`} type="text" inputMode="numeric" maxLength={2} value={value.frequency} onChange={e => update({ frequency: e.target.value })} aria-invalid={!!value.frequency && !frequencyCount(value.frequency)}/></div>}
